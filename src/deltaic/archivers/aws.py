...
class AWSArchiver(Archiver):
    ...
    MONTHLY_FREE_RETRIEVAL_FRACTION = 0.05
    ...
    def _request_download_quota(self, max_rate, download_size):
        ...
        size_per_hour = int(math.ceil(download_size / self.BILLING_RETRIEVAL_HOURS))
        ...
        data["max-bandwidth-month"] = str(
            max(timeslot_bw[0] + allocation, int(data["max-bandwidth-month"]))
        )

        # Update item
        try:
            with _conditional_check():
                self._domain.put_attributes(
                    self.BANDWIDTH_ITEM, data, expected_value=("serial", last_serial)
                )
        ...
        return (cur_slot, min(allocation * self.BILLING_RETRIEVAL_HOURS, download_size))
    ...
    def _seconds_to_next_timeslot(cls, timeslot):
        ...
        if wait_time > timedelta(0):
            return wait_time.seconds + 0.000001 * wait_time.microseconds
        else:
            return 0
    ...
    def list_sets(self):
        ...
        for metadata in self._domain.select(qstr, consistent_read=True):
            ...
            if creation_time and (
                now - dateutil.parser.parse(creation_time) < self.PROTECTED_PERIOD
            ):
                set["protected"] = True
    ...
    def delete_set(self, set_name):
        ...
        aids = []
        qstr = (
            "select `aws-archive`, `aws-aid` from `%s` where "
        ...
        for res in list(self._domain.select(qstr, consistent_read=True)):
            item_name = self._make_archive_item_name(set_name, res["aws-archive"])
    ...
    def upload_archive(self, set_name, archive_name, metadata, local_path):
        ...
        aid = self._vault.concurrent_create_archive_from_file(
            local_path,
            " ".join([set_name, archive_name]),
        ...
        try:
            ...
            with _conditional_check():
                self._domain.put_attributes(
                    item_name, metadata, expected_value=("aws-aid", False)
                )
        except _ConditionalCheckFailed:
    ...
    def _finish_download(self, queue, archive_name, job_id, local_path, offset):
        # Thread function
        try:
